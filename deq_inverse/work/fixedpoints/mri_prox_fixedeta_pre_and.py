...
parser.add_argument('--n_epochs', default=85)
parser.add_argument('--batch_size', type=int, default=4)  # Fix memory issues
parser.add_argument('--and_maxiters', default=30)  # REDUCED: More stable convergence (was 40)
...
parser.add_argument('--lr', type=float, default=0.0001)  # REDUCED: Fine-tuning for final epochs
parser.add_argument('--etainit', type=float, default=0.2)  # OPTIMIZED: Higher step size for better gradient steps toward optimal solution
parser.add_argument('--lr_gamma', type=float, default=0.5)  # Less aggressive decay for sustained learning
parser.add_argument('--sched_step', type=int, default=20)  # MORE FREQUENT: Fine-tune learning rate more often in final phase
...
load_location = r"G:\AK\dncnn_model\dncnn_mri_sigma001_optimized_final_2_final.ckpt"
...
trainset_size = 1250  # INCREASED: Use more data than paper (was 1250)
...
learned_component = DnCNN(n_channels, num_of_layers=17, lip=1.0)  # Keep your DnCNN config
...
# FIX: More stable Anderson acceleration parameters to prevent singular matrix
deep_eq_module = eq_utils.DEQFixedPoint(solver, forward_iterator, m=anderson_m, beta=anderson_beta, lam=0.08,
                                        max_iter=max_iters, tol=1e-4)  # More conservative parameters for stability
